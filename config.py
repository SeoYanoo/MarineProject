...
YOLO_CONF_THRESHOLD = _env_float("DETECTION_CONFIDENCE", 0.50, 0.05, 0.95)
NMS_IOU_THRESHOLD = _env_float("DETECTION_NMS_IOU", 0.35, 0.05, 0.95)
MAX_DETECTIONS_PER_FRAME = _env_int("MAX_DETECTIONS_PER_FRAME", 40, 1, 300)

# 영상 처리량 제한. 원본 재생 시간은 유지하면서 출력/추론 프레임만 줄인다.
VIDEO_INFERENCE_FPS = _env_float("VIDEO_INFERENCE_FPS", 3.0, 0.2, 15.0)
VIDEO_OUTPUT_FPS = _env_float("VIDEO_OUTPUT_FPS", 15.0, 1.0, 30.0)
VIDEO_MAX_INFERENCE_CALLS = _env_int("VIDEO_MAX_INFERENCE_CALLS", 180, 10, 1000)
VIDEO_MAX_DIMENSION = _env_int("VIDEO_MAX_DIMENSION", 1280, 480, 1920)
INFERENCE_MAX_DIMENSION = _env_int("INFERENCE_MAX_DIMENSION", 960, 480, 1920)
